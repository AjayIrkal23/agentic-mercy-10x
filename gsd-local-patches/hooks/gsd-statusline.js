#!/usr/bin/env node
// gsd-hook-version: 1.42.3-cursor
// Cursor CLI Statusline - GSD Edition
...
/** Cursor config dir; falls back to Claude dir during migration. */
function getConfigDir() {
  return process.env.CLAUDE_CONFIG_DIR || path.join(os.homedir(), '.claude');
}

/**
 * Compute context usage % for the progress bar.
 * Cursor CLI exposes used_percentage directly; Claude Code uses remaining_percentage
 * with an auto-compact buffer. Prefer Cursor fields when present.
 */
function computeContextUsage(data) {
  const cw = data.context_window || {};
  if (cw.used_percentage != null && !Number.isNaN(cw.used_percentage)) {
    return Math.max(0, Math.min(100, Math.round(cw.used_percentage)));
  }

  const remaining = cw.remaining_percentage;
  if (remaining == null || Number.isNaN(remaining)) return null;

  const totalCtx = cw.context_window_size || cw.total_tokens || 1_000_000;
...
/** Format a 10-segment colored context meter for the statusline. */
...
function runStatusline() {
  ...
  try {
    ...
    const model = data.model?.display_name || 'Cursor';
    const dir = data.workspace?.current_dir || data.workspace_roots?.[0] || data.cwd || process.cwd();
    const session = data.conversation_id || data.session_id || '';
    const remaining = data.context_window?.remaining_percentage;
    const used = computeContextUsage(data);
    let ctx = formatContextMeter(used);

    // Write context metrics to bridge file for gsd-context-monitor.js (PostToolUse).
    const sessionSafe = session && !/[/\\]|\.\./.test(session);
    if (sessionSafe && used != null) {
      try {
          const bridgePath = path.join(os.tmpdir(), `gsd-ctx-${session}.json`);
        const bridgeRemaining = remaining != null
          ? remaining
          : Math.max(0, 100 - used);
        const bridgeData = JSON.stringify({
          session_id: session,
          remaining_percentage: bridgeRemaining,
          used_pct: used,
    ...
    // Current task from todos (Cursor/Claude agent todo files, if present)
    let task = '';
    ...
    const legacyCacheFile = path.join(configDir, 'cache', 'gsd-update-check.json');
...
module.exports = {
  ...
  composeStatusline, computeContextUsage, formatContextMeter, getConfigDir,
};
