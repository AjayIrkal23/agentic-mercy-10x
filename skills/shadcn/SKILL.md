...
<!-- ponytail: `add --all` installs the entire kit (some components go unused). Deliberate per user directive — full, current kit always available beats minimal footprint. The dead-code/change audit may flag unused components; that's the accepted tradeoff. Narrow to `add <needed...>` from the step-1 live list if a project should stay lean. -->
