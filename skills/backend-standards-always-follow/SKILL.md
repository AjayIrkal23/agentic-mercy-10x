...
description: Use when any backend API, route, controller, schema, service, contract,
  persistence, auth, validation, worker, queue, integration, server/Fastify behavior,
  backend bugfix, or implementation is requested. Backend Core Compliance Set member
  for the always-on backend baseline As part of the Backend Core Compliance Set for
  any backend/server trigger surface before planning, debugging, implementation, review,
  delegation, or completion acceptance.
